...
def load():
 with LOCK:
  ...
  try:
   ...
   for row in rows: row.setdefault("priority","medium")
   return rows
...
class Create(BaseModel):
 title:str=Field(min_length=3,max_length=200); description:str=Field(min_length=3,max_length=5000); assignee:str; priority:str="medium"
class Update(BaseModel):
 title:str|None=Field(None,min_length=3,max_length=200); description:str|None=Field(None,min_length=3,max_length=5000); status:str|None=None; priority:str|None=None
...
def update(tid:str,p:Update,user:UserContext=Depends(require_devops)):
 ...
 if p.priority and p.priority not in VALID_PRIORITIES: raise HTTPException(400,"Priority must be low, medium, high or critical")
 old_status=t["status"]; old_priority=t.get("priority","medium")
 for k,v in p.model_dump(exclude_none=True).items(): t[k]=v.strip() if isinstance(v,str) else v
 t["updated_at"]=now()
 if p.status and p.status!=old_status: action=f"Status changed from {old_status} to {t['status']}"
 elif p.priority and p.priority!=old_priority: action=f"Priority changed from {old_priority} to {t['priority']}"
 else: action="Task updated"
 t["history"].append({"at":t["updated_at"],"actor":user.username,"action":action});save(rows);return t
