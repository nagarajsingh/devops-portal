...
def create_pipeline_request(payload: PipelineRequestCreate, user: UserContext) -> PipelineRequest:
    ...
    try:
        send_app_owner_approval_email(item.model_dump()); item = _record_notification_event(request_id, "Approval Email Sent", app_owner)
    except Exception as exc:
        logger.exception("Unable to send app owner email request_id=%s app_owner=%s", request_id, app_owner); item = _record_notification_event(request_id, "Approval Email Failed", str(exc))
    return item
...
def _apply_app_owner_decision(request_id: str, decision: str, app_owner: str, comment: str | None = None, source: str = "approval link") -> tuple[str, str]:
    ...
    current.update({"status": status, "app_owner_decision_by": app_owner, "app_owner_decision_at": decided_at, "app_owner_comment": clean_comment or None, "updated_at": decided_at})
    current.setdefault("timeline", []).append(timeline_event(action, app_owner, f"{detail} via {source}")); items[index] = current; write_requests(items)
...
def update_pipeline_request(request_id: str, payload: ReviewUpdate, user: UserContext) -> PipelineRequest:
    ...
    updated_values = payload.model_dump(exclude={"review_comments", "app_owner"}); updated_values["reference_branch"] = reference_branch; updated_values["target_cluster"] = target_cluster
    updated = {**current, **updated_values, "app_owner": current.get("app_owner") or APP_OWNER_EMAILS.get(payload.application_type, ""), "review_comments": payload.review_comments, "reviewed_by": user.username, "updated_at": now_iso(), "original_request": original}
    updated.setdefault("timeline", []).append(timeline_event("Modified by DevOps", user.username, payload.review_comments or "Request values updated")); items[index] = updated; write_requests(items)
...
def _save_provisioning_result(items: list[dict], index: int, current: dict, final_status: str, steps: dict) -> None:
    ...
    current.setdefault("timeline", []).append(timeline_event(final_status, "system", "Provisioning workflow paused" if final_status == "Pending Action" else "Provisioning workflow finished"))
    items[index] = current; write_requests(items)


def _raise_confirmation_if_needed(current: dict, steps: dict) -> None:
    repository_step = steps.get("repository", {})
    if repository_step.get("requires_confirmation"):
        raise HTTPException(status_code=409, detail={"code": "existing_repository_confirmation_required", "message": "Repository already exists. Create the isolated pipeline branch from the selected reference repository without modifying existing branches or code?", "request": current})
    pipeline_step = steps.get("pipeline", {})
    if pipeline_step.get("requires_confirmation"):
        raise HTTPException(status_code=409, detail={"code": "existing_build_pipeline_confirmation_required", "message": pipeline_step.get("message") or "Build pipeline already exists. Proceed with release pipeline creation using it?", "request": current})
...
def approve_pipeline_request(request_id: str, user: UserContext, azure_devops_pat: str) -> PipelineRequest:
    ...
    current.update({"status": "Provisioning", "reviewed_by": user.username, "updated_at": now_iso()}); current.setdefault("timeline", []).append(timeline_event("Approved by DevOps", user.username, "Provisioning started"))
    final_status, steps = provision(current, azure_devops_pat); _save_provisioning_result(items, index, current, final_status, steps); _raise_confirmation_if_needed(current, steps)
    return PipelineRequest(**current)
...
def confirm_existing_repository_bootstrap(request_id: str, user: UserContext, azure_devops_pat: str) -> PipelineRequest:
    ...
    final_status, steps = provision(current, azure_devops_pat); current.pop("allow_existing_repo_bootstrap", None); _save_provisioning_result(items, index, current, final_status, steps); _raise_confirmation_if_needed(current, steps)
    return PipelineRequest(**current)
...
def confirm_existing_build_pipeline(request_id: str, user: UserContext, azure_devops_pat: str) -> PipelineRequest:
    items = read_requests(); index, current = find_request(items, request_id)
    if current.get("status") != "Pending Action": raise HTTPException(status_code=409, detail=f"Request cannot continue from status {current.get('status')}")
    pipeline_step = current.get("provisioning", {}).get("pipeline", {})
    if not pipeline_step.get("id"): raise HTTPException(status_code=409, detail="No existing build pipeline is waiting for confirmation")
    if not azure_devops_pat.strip(): raise HTTPException(status_code=400, detail="Azure DevOps PAT is required for provisioning")
    current["allow_existing_repo_bootstrap"] = True; current["allow_existing_pipeline_release"] = True; current["status"] = "Provisioning"; current["updated_at"] = now_iso(); current.setdefault("timeline", []).append(timeline_event("Existing Build Pipeline Confirmed", user.username, f"DevOps confirmed reuse of build pipeline {pipeline_step.get('name') or pipeline_step.get('id')} for release pipeline creation"))
    final_status, steps = provision(current, azure_devops_pat); current.pop("allow_existing_repo_bootstrap", None); current.pop("allow_existing_pipeline_release", None); _save_provisioning_result(items, index, current, final_status, steps)
    return PipelineRequest(**current)
