...
def _refresh_build_run(run: dict[str, Any], pat: str) -> None:
    ...
    if not run_id:
        return
    code, body = azdo_request("GET", f"_apis/build/builds/{run_id}?api-version=7.1", pat)
    if code != 200:
...
def reextract_document(request_id: str, actor: str) -> dict[str, Any]:
    ...
    extracted_fields = (
        "filename", "application", "branch_name", "environment", "repository",
        "target_branch", "build_pipeline", "war_files", "jar_files",
        "container_images", "text_preview", "confidence", "collections_items", "country",
    )
...
def trigger_collections_build(request_id: str, actor: str, pat: str, updates: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in selected:
        pipeline_name = item.get("pipeline_name") or dm.COLLECTIONS_PIPELINES.get(row.get("country", ""), {}).get(item.get("service", ""), "")
        if not pipeline_name:
            raise HTTPException(status_code=422, detail=f"Pipeline mapping is missing for {item.get('service', 'service')}")
        ...
        if use_vendor_image and not vendor_image:
            raise HTTPException(status_code=422, detail=f"vendorImage is required for {item.get('service', 'service')}")
        parameters = {"vendorImage": vendor_image, "useVendorImage": use_vendor_image}
...
def perform_action(
    ...
    updates = updates or {}
    requires_pat = {
        "trigger-code-pull", "create-pr", "trigger-build", "trigger-deployment", "refresh-status",
    }
    if action in requires_pat and not pat:
        raise HTTPException(
            status_code=422,
