...
ARCHITECTURES: dict[str, dict[str, Any]] = {
    ...
    "Collections": {"code_pull": False, "pull_request": False, "build": True, "deployment": True},
...
REPO_MAPPING = {
    ...
    "oblm": "oblm-r2", "oblmic": "oblm-ic-r2", "obvam": "obvam-r2",
    "obvamic": "obvam-ic-r2",
}
...
def _structured_extract(filename: str, raw: bytes, application_type: str = "") -> dict[str, Any]:
    ...
    return {
        ...
        "confidence": {name: ("high" if value else "missing") for name, value in {
            "application": application, "branch_name": branch, "environment": environment
        }.items()},
    }


def extract_release_document(upload: UploadFile, raw: bytes, application_type: str = "") -> dict[str, Any]:
    ...
    return _structured_extract(upload.filename or "release-document", raw, application_type)


def submit_document(application_type: str, app_owner: str, upload: UploadFile, raw: bytes, requested_by: str) -> dict[str, Any]:
    ...
    row = {
        ...
        "app_owner": app_owner.strip(), "document_name": upload.filename or "release-document",
        "document_path": str(document_path), "extracted": False, "architecture": architecture,
        "application": "", "branch_name": "", "environment": "", "repository": "",
        "target_branch": "", "build_pipeline": "", "war_files": [], "jar_files": [],
        "container_images": [],
        "steps": {
            "owner_approval": {"status": "Pending"}, "devops_review": {"status": "Waiting"},
            "document_extraction": {"status": "Waiting"}, "code_pull": {"status": "Not Required" if not architecture.get("code_pull") else "Waiting"},
        ...
        "timeline": [{"at": now, "action": "Document submitted", "actor": requested_by}],
    }
...
def update_action(request_id: str, action: str, actor: str, pat: str = "", updates: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if action == "owner-approve":
        ...
        path = Path(row["document_path"]); extracted = _structured_extract(row["document_name"], path.read_bytes(), row["application_type"])
        ...
        for key in ("application", "branch_name", "environment", "repository", "target_branch", "build_pipeline", "war_files", "jar_files", "container_images"):
        ...
        steps["build"] = _pipeline_run(pipeline_name, parameters, pat); row["status"] = "Build Running"
        ...
        steps["deployment"] = _pipeline_run(pipeline_name, updates.get("parameters") or {"environment": row.get("environment"), "application": row.get("application")}, pat)
        row["status"] = "Deployment Running"
    else:
        raise HTTPException(status_code=400, detail="Unsupported deployment action")
    row["updated_at"] = now; row.setdefault("timeline", []).append({"at": now, "action": action, "actor": actor}); _save(rows)
    return row
