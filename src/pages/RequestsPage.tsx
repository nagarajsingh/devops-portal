...
export default function RequestsPage({ token, role, refreshKey }: Props) {
  ...
  useEffect(() => {
    ...
    getIngresses(draft.namespace, token)
        ...
        setDraft((current) =>
          current
            ? {
                ...current,
                ingress_name:
                  current.ingress_name && items.includes(current.ingress_name)
                    ? current.ingress_name
                    : "",
              }
            : current,
        );
      ...
      .catch((reason) => {
        setIngresses([]);
        setError(reason instanceof Error ? reason.message : "Unable to load ingresses");
      })
      ...
      .then((items) => {
        ...
        setDraft((current) => {
          ...
          if (service) {
            return {
              ...current,
              service_port: service.ports.includes(current.service_port)
                ? current.service_port
                : service.ports[0] ?? current.service_port,
            };
          }
  ...
  useEffect(() => {
    if (previewLoading && !loadingIngresses && !loadingServices) {
  ...
  const originalChanges = useMemo(() => {
    ...
    return Object.entries(selected.original_request)
      ...
      .map(([key, value]) => ({
        ...
        approved: String(selected[key as keyof PipelineRequest] ?? ""),
      }));
  }, [selected]);

  const openPreview = (request: PipelineRequest) => {
    setOperationStatus("Opening request preview...");
    ...
    setDraft({
      ...
      setup_pipeline: request.setup_pipeline ?? Boolean(request.reference_repository_name),
  ...
  const saveChanges = async () => {
    ...
    setOperationStatus("Saving DevOps changes...");
    setBusy(true);
    setError("");
    try {
  ...
  const approve = async () => {
    if (!selected || !draft) return;
    if (draft.reference_repository_name.trim() && !draft.reference_branch?.trim()) {
      setError("Provide the reference repository branch before approval.");
      return;
    }
    if (draft.setup_pipeline && !draft.reference_repository_name.trim()) {
      setError("Pipeline setup requires a reference repository.");
      return;
    }
    if (!draft.ingress_name) {
      setError("Select an ingress from the requested namespace before approval.");
      return;
    }
    if (!draft.service_name.trim()) {
      setError(
        draft.create_service
          ? "Provide the Kubernetes service name."
          : "Select an existing Kubernetes service.",
      );
      return;
    }
    if (!azureDevOpsPat.trim()) {
      setError("Provide your Azure DevOps PAT before approval.");
      return;
    }
    ...
    const stages = [
      ...
      draft.reference_repository_name.trim()
        ? "Creating or updating feature/devops branch and copying template files..."
        : "Reference repository not selected; skipping template copy...",
      draft.setup_pipeline
        ? "Creating Azure DevOps build pipeline..."
        : "Pipeline setup disabled; skipping pipeline creation...",
      draft.create_service
        ? "Creating Kubernetes service..."
        : "Validating selected Kubernetes service...",
    ...
    try {
      ...
      setSelected(updated);
      setNotice(
        `Provisioning finished with status: ${updated.status}. Review the results and close the ticket with a mandatory comment.`,
      );
      await load();
    } catch (reason) {
      setAzureDevOpsPat("");
      ...
      setError(
        message.includes("already exists")
          ? `${message}. Rename the repository and save before approving again.`
          : message,
      );
      await load();
    } finally {
  ...
  const reject = async () => {
    ...
    if (!reason) return;
    setOperationStatus("Rejecting request...");
    setBusy(true);
    try {
  ...
  const closeTicket = async () => {
    ...
    if (!comment) {
      setError("Closure comment is mandatory.");
      return;
    }

    setOperationStatus("Closing ticket and publishing provisioning details...");
    setBusy(true);
    setError("");
    try {
      const updated = await closePipelineRequest(selected.id, comment, token);
      setSelected(updated);
      setShowClosureForm(false);
      setClosureComment("");
      setNotice("Ticket closed. Repository and pipeline details are now visible to the developer.");
      await load();
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : "Unable to close ticket");
    } finally {
      setBusy(false);
      setOperationStatus("");
    }
    ...
    <section>
      {(busy || previewLoading) && (
        <div className="operation-overlay">
          ...
          <strong>{operationStatus || (busy ? "Processing request..." : "Loading preview...")}</strong>
          <span>Please wait. Do not refresh or close this page.</span>
        </div>
      )}
            ...
            <tbody>
              {requests.map((request) => (
                <tr key={request.id}>
                  ...
                  <td>{request.setup_pipeline ? "Yes" : "No"}</td>
      ...
      {selected && draft && (
        <div className="request-modal-backdrop" onClick={() => !busy && setSelected(null)}>
          <article className="request-modal" onClick={(event) => event.stopPropagation()}>
            <div className="modal-header">
              <div>
                <span className="request-id">{selected.id}</span>
                <h2>{selected.repository_name}</h2>
                <p>Submitted by {selected.requested_by} on {new Date(selected.created_at).toLocaleString()}</p>
              </div>
              <button className="secondary-button" disabled={busy} onClick={() => setSelected(null)}>Close</button>
            </div>
            ...
            <div className="form-card two-column review-form">
              ...
              <label>Reference Repository (Optional)<input disabled={role !== "devops"} value={draft.reference_repository_name} onChange={(e) => { const value = e.target.value; setDraft({ ...draft, reference_repository_name: value, setup_pipeline: Boolean(value.trim()), reference_branch: value.trim() ? draft.reference_branch : "" }); }} /></label>
            ...
            {showClosureForm && role === "devops" && closableStatuses.includes(selected.status) && (
              <div className="ticket-closure-form">
                <div>
                  <span className="eyebrow">FINAL REVIEW</span>
                  <h3>Close this ticket</h3>
                  <p>Add a clear closure comment. This comment, repository URL and pipeline URL will be visible to the developer.</p>
                </div>
                <label>
                  ...
                  <textarea
                    ...
                    placeholder="Example: Repository, pipeline, service and ingress were created and verified successfully."
                    onChange={(event) => setClosureComment(event.target.value)}
                  />
                  <small>{closureComment.trim().length}/1000 characters</small>
                </label>
                <div className="ticket-closure-actions">
                  <button className="secondary-button" disabled={busy} onClick={() => { setShowClosureForm(false); setClosureComment(""); setError(""); }}>Cancel</button>
                  <button className="primary-button" disabled={busy || !closureComment.trim()} onClick={closeTicket}>Confirm & Close Ticket</button>
                </div>
              </div>
            )}
            ...
            {selected.status === "Closed" && (
              <div className="closure-summary">
                <h3>Provisioning Completed and Ticket Closed</h3>
                <p><strong>DevOps closure comment:</strong> {selected.closure_comment}</p>
                <div className="resource-links">
                  <div><span>Repository</span>{repositoryUrl ? <a href={repositoryUrl} target="_blank" rel="noreferrer">Open repository</a> : <strong>Not created or unavailable</strong>}</div>
                  <div><span>Build Pipeline</span>{pipelineUrl ? <a href={pipelineUrl} target="_blank" rel="noreferrer">Open pipeline</a> : <strong>Not created or skipped</strong>}</div>
                </div>
              </div>
            )}
            ...
            {role === "devops" && closableStatuses.includes(selected.status) && !showClosureForm && (
              <div className="modal-actions">
                {editableStatuses.includes(selected.status) && (
                  <>
                    <button disabled={busy} className="secondary-button" onClick={saveChanges}>Save Changes</button>
                    <button disabled={busy} className="danger-button" onClick={reject}>Reject</button>
                    <button disabled={busy} className="primary-button" onClick={approve}>Approve & Provision</button>
                  </>
                )}
                <button disabled={busy} className="secondary-button close-ticket-button" onClick={openClosureForm}>Close Ticket</button>
              </div>
            )}
          </article>
        </div>
