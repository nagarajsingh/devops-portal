...
export default function RequestsPage({ token, role, refreshKey }: Props) {
  ...
  useEffect(() => { if (previewLoading && !loadingIngresses && !loadingServices) setPreviewLoading(false); }, [previewLoading, loadingIngresses, loadingServices]);
  ...
  const openPreview = (request: PipelineRequest) => {
    setPreviewLoading(true); setSelected(request); setNotice(""); setError(""); setAzureDevOpsPat("");
  ...
  const approve = async () => {
    ...
    try { await updatePipelineRequest(selected.id, draft, token); const updated = await approvePipelineRequest(selected.id, azureDevOpsPat, token); setAzureDevOpsPat(""); setSelected(updated); setNotice(`Provisioning finished with status: ${updated.status}`); await load(); }
  ...
  const reject = async () => { if (!selected) return; const reason = window.prompt("Provide rejection reason"); if (!reason) return; setBusy(true); try { const updated = await rejectPipelineRequest(selected.id, reason, token); setSelected(updated); setNotice("Request rejected."); await load(); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Unable to reject request"); } finally { setBusy(false); } };
  const closeTicket = async () => { if (!selected) return; const comment = window.prompt("Mandatory closure comment"); if (!comment?.trim()) { setError("Closure comment is mandatory."); return; } setBusy(true); try { const updated = await closePipelineRequest(selected.id, comment.trim(), token); setSelected(updated); setNotice("Ticket closed by DevOps."); await load(); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "Unable to close ticket"); } finally { setBusy(false); } };
  const selectedService = services.find((item) => item.name === draft?.service_name);

  return <section>
    {(busy || previewLoading) && <div className="operation-overlay"><div className="loading-spinner"/><strong>{busy ? "Processing request..." : "Loading preview..."}</strong></div>}
    ...
    {selected && draft && <div className="request-modal-backdrop" onClick={() => !busy && setSelected(null)}><article className="request-modal" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="form-card two-column review-form">
        ...
        {role === "devops" && <label className="full-width">Azure DevOps PAT<input type="password" autoComplete="new-password" value={azureDevOpsPat} placeholder="PAT is used once and is never stored" onChange={(e) => setAzureDevOpsPat(e.target.value)} /><small>Required only when approving.</small></label>}
      ...
      {role === "devops" && ["Pending Approval","Pending Action","Partially Completed"].includes(selected.status) && <div className="modal-actions"><button disabled={busy} className="secondary-button" onClick={saveChanges}>Save Changes</button><button disabled={busy} className="danger-button" onClick={reject}>Reject</button><button disabled={busy} className="secondary-button" onClick={closeTicket}>Close Ticket</button><button disabled={busy} className="primary-button" onClick={approve}>Approve & Provision</button></div>}
    </article></div>}
  </section>;
}
