...
export default function App() {
  ...
  const visibleItems = useMemo(
    () => navItems.filter((item) => !item.devopsOnly || session?.role === "devops"),
    [session],
  );
      ...
      <aside className="sidebar">
        ...
        <nav>
          {visibleItems.map((item) => {
            ...
            const active = !item.href && page === item.key;
            return (
              <button key={item.key} className={active ? "nav-item active" : "nav-item"} onClick={() => navigate(item)}>
                <Icon size={20} />
        ...
        <div className="sidebar-footer">
          ...
          <div>
            ...
            <span>{session.role === "devops" ? "DevOps" : "Developer"}</span>
          </div>
        </div>
      </aside>
      ...
      <main className="main-area">
        <header className="topbar">
          <div className="search-wrap">
            <Search size={20} />
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search tasks, services and requests..." />
            {query && (
              <div className="search-results">
                {filteredItems.map((item) => (
                  <button key={item.key} onClick={() => navigate(item)}>{item.label}</button>
                ))}
              </div>
            )}
          </div>
          <div className="topbar-actions">
            <NotificationCenter
              ...
              onOpenRequests={() => changePage("requests")}
            />
